...
const FundTrans = lazy(() => import("./components/Account/fundtans.js"))
const HRSetup = lazy(() => import("./components/TimecardSetup/HrSetup.js"));
const Home = lazy(() => import("./components/Home"));
const Dashboard = lazy(() => import("./components/Dashboard"));
const Reports = lazy(() => import("./components/Reports/Reports"));
const PayrollReports = lazy(
  () => import("./components/Reports/PayrollReports")
);

const SupervisorSetup = lazy(() => import("./components/SupervisorSetup/SupervisorSetup.js"))
const TimeCardReports = lazy(
  () => import("./components/Reports/TimeCardReports")
)
...
const EmployeeLeaveRequest = lazy(
  () => import("./components/TimecardEmployee/EmployeeLeaveRequest/index.js")
);
...
const PayRaise = lazy(
  () => import("./components/Payroll/PayRaise")
);
...
const MyIHPOComponnent = lazy(() => import("./components/PurchaseOrder/MyIHPO"))
...
const JobDescriptions = lazy(
  () => import("./components/Payroll/JobDescriptions.js")
);

const Loaders = lazy(() => import("../src/components/Loaders.js"));

function App() {
  const [isLoaders, setLoaders] = React.useState(0);
  ...
  useEffect(() => {
    const requestInterceptors = axiosInstance.interceptors.request.use(
      (config) => {
        setLoaders((prev) => {
          ...
          if (updated > 0) setIsLoading(true);
          return updated;
        });
        return config;
      }
    ...
    const responseInterceptors = axiosInstance.interceptors.response.use(
      (config) => {
        setLoaders((prev) => {
          const updated = prev - 1;
          if (updated <= 0) setIsLoading(false);
          return updated;
        });
  ...
  return (
    <Suspense>
      {isLoading && <Loaders show={isLoaders} />}
      <NotificationHandler />
      ...
      <Routes>
        ...
        <Route element={<RootLayout />}>
          ...
          <Route path="/SubstituteSetup" element={<SupervisorSetup />} />
          ...
          <Route path="/my-ihpo" element={<MyIHPOComponnent />} />
          ...
          <Route path="/timecardReports" element={<TimeCardReports />} />
          ...
          <Route path="/function-resource" element={<FunctionalDemo />} />
          <Route
            path="/FunctionalDemoResourceData"
            element={<FunctionalDemoResourceData />}
          />
          <Route
            path="/payroll/Job-descriptions"
