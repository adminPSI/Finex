...
export const DatePickerCell = (props) => {
  ...
  let myDate;
  if (data.date) {
    myDate = data.date ? data.date.toISOString() : null;
    showDate = data.date ? data.date : null;
  } else if (data.invoiceDate) {
    myDate = data.invoiceDate ? data.invoiceDate.toISOString() : null;
    showDate = data.invoiceDate ? data.invoiceDate : null;
  }
  if (!data.inEdit) {
