...
export default function IHCAccountCode() {
  ...
  const [IHCAccountData, setIHCAccountData] = useState([]);
  const [addIHCAccountVisible, setAddIHCAccountVisible] = useState(false);
  const [formInit, setFormInit] = useState([]);
  const [, setIHCAccountDropdownData] = useState([]);
  ...
  const [bindIHCAccountGrid, setBindIHCAccountGrid] = useState(null);

  React.useEffect(() => {
    const getData = setTimeout(() => {
      if (bindIHCAccountGrid) {
        BindIHCAccountGrid(
          bindIHCAccountGrid.code,
          bindIHCAccountGrid.description,
          bindIHCAccountGrid.revenueCheck,
          bindIHCAccountGrid.expenseCheck,
          bindIHCAccountGrid.salaryBenefits,
          bindIHCAccountGrid.startDate,
          bindIHCAccountGrid.isActive,
          bindIHCAccountGrid.search,
          bindIHCAccountGrid.cskip,
          bindIHCAccountGrid.ctake == "All" ? 0 : bindIHCAccountGrid.ctake,
          bindIHCAccountGrid.desc,
          bindIHCAccountGrid.sortKey
        );
      }
    }, 500);
  ...
  const BindIHCAccountGrid = (
    ...
    axiosInstance({
      ...
      .then((response) => {
        ...
        setIHCAccountData(data);

        let itemsData = [];
        data.forEach((data) => {
          let items = {
            text: data.IHCAccountCode,
            id: data.id,
          };
          itemsData.push(items);
        });
        setIHCAccountDropdownData(itemsData);
        if (
          bindIHCAccountGrid.isActive &&
          bindIHCAccountGrid.isActive == isActive
        ) {
  ...
  const addIHCAccountToggleDialog = () => {
    setAddIHCAccountVisible(!addIHCAccountVisible);

    if (addIHCAccountVisible) {
      setFormInit([]);
    }
  };

  const addIHCAccountCodeHandleSubmit = async (dataItem, e) => {
    ...
    if (dataItem.edit) {
      try {
        await axiosInstance({
          method: "PUT",
          url: IHCAccountEndPoints.AddIHCAccount + "?id=" + dataItem.id,
          data: apiRequest,
          withCredentials: false,
        });
        setshowInactive(false);
        ...
        addIHCAccountToggleDialog();
      } catch (error) { }
    } else {
      try {
        await axiosInstance({
          method: "POST",
          url: IHCAccountEndPoints.AddIHCAccount,
          data: apiRequest,
          withCredentials: false,
        });
        setshowInactive(false);
  ...
  //open ContextMenu for IHCAccount Grid
  ...
  //close ContextMenu for IHCAccount Grid
  ...
  const addInactiveHandleSubmit = (dataItem, e) => {
    ...
    axiosInstance({
      method: "GET",
      url: IHCAccountEndPoints.GetIHCAccount + "/" + dataItem.id,
      withCredentials: false,
    })
      .then((response) => {
        ...
        axiosInstance({
          ...
          .then((response) => {
            ...
            showSuccessNotification("IHAC account inactivated successfully");
          })
          .catch(() => { });
      })
      .catch(() => { })
    ...
    if (id !== 0) {
      switch (e.item.data.action) {
        case "edit":
          axiosInstance({
            method: "GET",
            url: IHCAccountEndPoints.GetIHCAccount + "/" + id,
            withCredentials: false,
          })
            .then((response) => {
              let data = response.data;
  ...
  //Click Event bind ContextMenu for IHCAccount Grid
  ...
  //Custom Cell to bind ContextMenu for IHCAccount Grid
  ...
  const { checkPrivialgeGroup, loading, error } = usePrivilege('Account')
  if (loading) return <div>Loading privileges...</div>
  if (error) return <div>Error: {error}</div>
  return (
    <>
      {checkPrivialgeGroup("AcctCodeM", 1) && (
        <>
          ...
          <div className="row">
            ...
            <div className="col-sm-4 text-end">
              {checkPrivialgeGroup("AddAccountCodeB", 2) && (
                <Button
                  themeColor={"primary"}
                  onClick={addIHCAccountToggleDialog}
                >
                  <i className="fa-solid fa-plus"></i> Add New Account Code
                </Button>
              )}
            </div>
          </div>
          {checkPrivialgeGroup("AcctCodeG", 1) && (
            <div className="mt-3">
              <Grid
                resizable={true}
                data={IHCAccountData}
              ...
              >
                <GridToolbar>
                  <div className="row col-sm-12">
                    ...
                    <div className="col-sm-6 d-flex align-items-center justify-content-center">
                      <div className="col-3">
                        {checkPrivialgeGroup("ShowAccountInactiveCB", 1) && (
                          <Checkbox
                            type="checkbox"
                            id="showInactive"
                            name="showInactive"
                        ...
                        {checkPrivialgeGroup("SMIAcctCodeCB", 1) && (
                          <Checkbox
                            type="checkbox"
                            id="modifiedBy"
                            name="modifiedBy"
                ...
                <Column field="code" title="Account Code" />
              ...
              >
                {checkPrivialgeGroup("EditAccountCM", 3) && !showInactive && (
                  <MenuItem
                    text="Edit Account"
                    data={{
                      action: "edit",
                ...
                {checkPrivialgeGroup("MakeAccountInactiveCM", 2) &&
                  !showInactive && (
                    <MenuItem
                      text="Make Account Inactive"
                      data={{
                        action: "inactive",
                      }}
                  ...
                  <div className="d-flex align-items-center justify-content-center">
                    {formInit?.id ? (
                      ...
                      <>
                        ...
                        <span className="ms-2">Add New Account</span>
                      </>
                    )}
                  </div>
                }
                onClose={addIHCAccountToggleDialog}
              >
                <Form
                  onSubmit={addIHCAccountCodeHandleSubmit}
