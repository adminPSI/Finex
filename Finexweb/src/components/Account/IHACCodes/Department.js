...
export default function DepartmentCode() {
  ...
  const [, setDepartmentDropdownData] = useState([]);
  ...
  const [bindDepartmentGrid, setBindDepartmentGrid] = useState(null);

  React.useEffect(() => {
    const getData = setTimeout(() => {
      if (bindDepartmentGrid) {
        BindDepartmentGrid(
          bindDepartmentGrid.code,
          bindDepartmentGrid.description,
          bindDepartmentGrid.revenueCheck,
          bindDepartmentGrid.expenseCheck,
          bindDepartmentGrid.salaryBenefits,
          bindDepartmentGrid.startDate,
          bindDepartmentGrid.isActive,
          bindDepartmentGrid.search,
          bindDepartmentGrid.cskip,
          bindDepartmentGrid.ctake == "All" ? 0 : bindDepartmentGrid.ctake,
          bindDepartmentGrid.desc,
          bindDepartmentGrid.sortKey
        );
      }
    }, 500);
  ...
  const BindDepartmentGrid = (
    ...
    axiosInstance({
      ...
      .then((response) => {
        ...
        if (
          bindDepartmentGrid.isActive &&
          bindDepartmentGrid.isActive == isActive
        ) {
  ...
  const addDepartmentToggleDialog = () => {
    setAddDepartmentVisible(!addDepartmentVisible);
  ...
  const addDepartmentCodeHandleSubmit = async (dataItem, e) => {
    ...
    if (dataItem.edit) {
      try {
        await axiosInstance({
          method: "PUT",
          url: DepartmentEndPoints.AddDepartment + "?id=" + dataItem.id,
          data: apiRequest,
          withCredentials: false,
        });
        setshowInactive(false);
        ...
        addDepartmentToggleDialog();
      } catch (error) {}
    } else {
      try {
        await axiosInstance({
          method: "POST",
          url: DepartmentEndPoints.AddDepartment,
          data: apiRequest,
          withCredentials: false,
        });
        setshowInactive(false);
  ...
  //open ContextMenu for department Grid
  ...
  //close ContextMenu for department Grid
    ...
    if (id !== 0) {
      switch (e.item.data.action) {
        case "edit":
          axiosInstance({
            method: "GET",
            url: DepartmentEndPoints.GetDepartment + "/" + id,
            withCredentials: false,
          })
            .then((response) => {
  ...
  const addInactiveHandleSubmit = (dataItem, e) => {
    ...
    axiosInstance({
      method: "GET",
      url: DepartmentEndPoints.GetDepartment + "/" + dataItem.id,
      withCredentials: false,
    })
      .then((response) => {
        let data = response.data;
        ...
        axiosInstance({
          ...
          .then((response) => {
            ...
            inactiveToggleDialog();
            showSuccessNotification("IHAC department inactivated successfully");
          })
          .catch(() => {});
      })
      .catch(() => {})
  ...
  //Click Event bind ContextMenu for department Grid
  const handleContextMenu = (props) => {
    handleContextMenuOpen(props);
  };

  //Custom Cell to bind ContextMenu for department Grid
  ...
  const { checkPrivialgeGroup, loading, error } = usePrivilege('Department')
  if (loading) return <div>Loading privileges...</div>
  if (error) return <div>Error: {error}</div>

  return (
    <>
      {checkPrivialgeGroup("DeptCodeM", 1) && (
        <>
          ...
          <div className="row">
            ...
            <div className="col-sm-4 text-end">
              {checkPrivialgeGroup("AddDepartmentCodeB", 2) && (
                <Button
                  themeColor={"primary"}
                  onClick={addDepartmentToggleDialog}
                >
                  <i className="fa-solid fa-plus"></i> Add New Department Code
                </Button>
              )}
            </div>
          </div>
          {checkPrivialgeGroup("DeptCodeG", 1) && (
            <div className="mt-3">
              <Grid
                resizable={true}
                data={departmentData}
              ...
              >
                <GridToolbar>
                  <div className="row col-sm-12">
                    ...
                    <div className="col-sm-6 d-flex align-items-center justify-content-center">
                      <div className="col-3">
                        {checkPrivialgeGroup("ShowDepartmentInactiveCB", 1) && (
                          <Checkbox
                            type="checkbox"
                            id="showInactive"
                            name="showInactive"
                        ...
                        {checkPrivialgeGroup("SMIDeptCB", 1) && (
                          <Checkbox
                            type="checkbox"
                            id="modifiedBy"
                            name="modifiedBy"
                ...
                <Column field="code" title="Department Code" />
              ...
              >
                {checkPrivialgeGroup("EditDepartmentCM", 3) &&
                  !showInactive && (
                    <MenuItem
                ...
                {checkPrivialgeGroup("MakeDepartmentInactiveCM", 2) &&
                  !showInactive && (
                    <MenuItem
                      text="Make department inactive"
                      data={{
                        action: "inactive",
                      }}
                  ...
                  <div className="d-flex align-items-center justify-content-center">
                    {formInit?.id ? (
                      ...
                      <>
                        ...
                        <span className="ms-2">Add New Department</span>
                      </>
                    )}
                  </div>
                }
                onClose={addDepartmentToggleDialog}
              >
                <Form
                  onSubmit={addDepartmentCodeHandleSubmit}
